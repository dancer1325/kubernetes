* `func AddSourceToErr(verb string, source string, err error) error {}`
  * | error message, 
    * adds `handleResourcePrefix` + source string
  * `verb string`
    * _Example:_ "creating", "deleting" etc.
  * `source string`
    * filename OR URL -- to the -- template file(*.json or *.yaml), OR
    * stdin -- to -- handle the resource
...
* `func AddServerSideApplyFlags(cmd *cobra.Command) {`
  * `server-side`
    * by default, false
    * if true -> apply runs | server 
      * instead of | client
  * `force-conflicts`
    * by default, false
    * if true -> apply force the changes -- against -- conflicts
